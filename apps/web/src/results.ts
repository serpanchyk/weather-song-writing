...
export function renderHistory(page: RunHistoryPage, compact = false): string {
  ...
  return runs.length
    ? `<div class="history-list">${runs.map(renderHistoryItem).join("")}</div>`
    : `<div class="empty-state"><h3>No comparisons yet</h3><p>Your completed weather-and-lyrics comparisons will appear here.</p></div>`;
}
...
function renderRubric(evaluation: CandidateJudgeEvaluation): string {
  const entries = [
    ["Instruction following", evaluation.scores.instructionFollowing],
    ["Lyrical quality", evaluation.scores.lyricalQuality],
    ["Creativity", evaluation.scores.creativity],
    ["Weather relevance", evaluation.scores.weatherRelevance],
  ] as const;
...
function renderHistoryItem(run: RunSummary): string {
  const location =
    run.input.location.kind === "city"
      ? run.input.location.city
      : `${run.input.location.latitude.toFixed(2)}, ${run.input.location.longitude.toFixed(2)}`;
...
function metric(value: number | null | undefined, label: string): string {
